...
/* Modified for use in eggdbus for gint16/guint16 support since the GObject
 * maintainer refuse to add GTypes for int16/uint16.
 *
 *  http://bugzilla.gnome.org/show_bug.cgi?id=562498
 */
...
#undef G_LOG_DOMAIN
#define G_LOG_DOMAIN "EggDBus-GLib-Genmarshal"
#include <glib.h>
...
/* --- defines --- */
#define	PRG_NAME	"eggdbus-glib-genmarshal"
#define	PKG_NAME	"EggDBus"
#define PKG_HTTP_HOME	"http://cgit.freedesktop.org/~david/eggdbus"
...
put_marshal_value_getters (void)
{
  ...
  fputs ("#define g_marshal_value_peek_int16(v)    egg_dbus_value_get_int16 (v)\n", fout);
  fputs ("#define g_marshal_value_peek_uint16(v)   egg_dbus_value_get_uint16 (v)\n", fout);
  ...
  fputs ("#define g_marshal_value_peek_int16(v)    (v)->data[0].v_int\n", fout);
  fputs ("#define g_marshal_value_peek_uint16(v)   (v)->data[0].v_uint\n", fout);
...
complete_in_arg (InArgument *iarg)
{
  static const InArgument args[] = {
    ...
    { "INT16",		"INT16",	"gint16",       "g_marshal_value_peek_int16",	},
    { "UINT16",		"UINT16",	"guint16",	"g_marshal_value_peek_uint16",	},
...
complete_out_arg (OutArgument *oarg)
{
  static const OutArgument args[] = {
    ...
    { "INT16",		"INT16",	"gint16",	"egg_dbus_value_set_int16",		     },
    { "UINT16",		"UINT16",	"guint16",	"egg_dbus_value_set_uint16",		     },
...
#include	"gmarshal.strings"
  ...
  if (std_includes)
    for (i = 0; (guint) i < G_N_ELEMENTS (gobject_marshallers); i++)
      {
